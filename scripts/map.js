(function () {
    ...
    let base = currentMode() === 'dark' ? darkTiles : lightTiles;
    base.addTo(map);

    // allow external toggle to switch tiles
    window.__setMapTheme = function (mode) {
        const wantDark = (mode === 'dark');
        const haveDark = base === darkTiles;
        if (wantDark !== haveDark) {
            map.removeLayer(base);
            base = wantDark ? darkTiles : lightTiles;
            base.addTo(map);
        }
    };

    // also react to system changes (when no manual override)
    const mq = window.matchMedia('(prefers-color-scheme: dark)');
    mq.addEventListener?.('change', e => {
        const manual = document.documentElement.dataset.theme;
        if (!manual) window.__setMapTheme(e.matches ? 'dark' : 'light');
    });
